...
def test_build_creates_structure_on_empty_tile(action_type, kind):
...
def test_build_noop_on_locked_tile():
...
def test_build_noop_on_occupied_tile(occupant):
    env = _make_env(farmer=(5, 5))
    farm = env.state.farms[0]
...
def test_build_noop_on_existing_structure_preserves_it():
    env = _make_env(farmer=(5, 5))
    farm = env.state.farms[0]
...
def test_build_noop_on_malformed_or_negative_position(bad_pos):
    env = _make_env(farmer=(5, 5))
    farm = env.state.farms[0]
...
def test_build_noop_out_of_bounds():
...
def test_step_dispatches_build_coop():
...
def test_step_dispatches_build_pasture():
    env = _make_env(farmer=(3, 3))
    step = StepState(
