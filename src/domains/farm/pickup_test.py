...
def test_pickup_noop_when_not_shed_adjacent():
    ...
    assert env.state.private.shed.WHEAT == 5  # unchanged
...
def test_pickup_noop_on_invalid_item():
    ...
    assert env.state.private.shed.WHEAT == 5  # untouched
...
def test_pickup_writes_to_the_correct_inventory_index():
    ...
    env.state.private.inventories = [{}, {"CARROT": 7}]

    pickup(env.state, farm, farm.farmer,
           PickupActionState(type="PICKUP", item="WHEAT", count=2), 0)

    assert env.state.private.inventories[0].get("WHEAT") == 2
    assert env.state.private.inventories[1].get("CARROT") == 7  # hand's untouched
    assert env.state.private.inventories[1].get("WHEAT") is None
