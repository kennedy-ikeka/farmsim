...
def test_refresh_plant_unwatered_increments_consecutive_unwatered():
...
def test_refresh_plant_watered_resets_consecutive_unwatered():
...
def test_refresh_plant_first_miss_when_planted_and_not_watered():
...
def test_refresh_plant_consecutive_unwatered_two_becomes_weed():
    env = _make_env(farmer=(5, 5), step=24)
    farm = env.state.farms[0]
...
def test_refresh_plant_watered_plant_survives_when_counter_was_one():
    env = _make_env(farmer=(5, 5), step=24)
    farm = env.state.farms[0]
    farm.tiles[5][5] = _plant(consecutive_unwatered=1, watered_today=True)
...
def test_refresh_plant_decay_drops_yield_every_other_turn_past_lifespan():
...
def test_refresh_plant_decay_skips_on_even_offset_past_lifespan():
...
def test_refresh_plant_decays_to_zero_becomes_weed():
...
def test_refresh_plant_decay_floor_at_zero_does_not_go_negative():
    env = _make_env(farmer=(5, 5), step=101)
    farm = env.state.farms[0]
...
def test_refresh_plant_no_decay_before_lifespan():
...
def test_refresh_plant_neglect_converts_to_weed_even_past_lifespan():
    env = _make_env(farmer=(5, 5), step=101)
    farm = env.state.farms[0]
...
def test_refresh_plant_works_for_every_crop(crop):
    env = _make_env(farmer=(5, 5), step=24)
    farm = env.state.farms[0]
