...
def _inv(env, idx=0):
    while len(env.state.private.inventories) <= idx:
        env.state.private.inventories.append({})
    return env.state.private.inventories[idx]
...
def test_place_animal_on_matching_empty_structure(animal, kind):
...
def test_place_animal_resets_fields_on_reused_structure():
...
def test_place_animal_count_is_ignored():
...
def test_place_animal_removes_inventory_entry_when_depleted():
    env = _make_env(farmer=(0, 0))
    farm = env.state.farms[0]
    farm.tiles[0][0] = _empty_structure(kind="COOP")
    _inv(env)["GOOSE"] = 1
...
def test_place_animal_noop_on_wrong_structure_kind():
    env = _make_env(farmer=(0, 0))
    farm = env.state.farms[0]
...
def test_place_animal_noop_when_structure_already_occupied():
    env = _make_env(farmer=(0, 0))
    farm = env.state.farms[0]
...
def test_place_animal_noop_when_not_in_inventory():
    env = _make_env(farmer=(0, 0))
    farm = env.state.farms[0]
    farm.tiles[0][0] = _empty_structure(kind="COOP")
...
def test_place_animal_noop_on_non_structure_tile():
    env = _make_env(farmer=(0, 0))
    farm = env.state.farms[0]
...
def test_place_shed_drop_moves_items_to_shed(pos):
...
def test_place_shed_drop_moves_only_available_when_count_exceeds_held():
...
def test_place_shed_drop_capped_by_shed_capacity():
    """Shed is near full; only the remaining capacity is accepted."""
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    env.state.private.shed.WHEAT = 98  # 2 slots left
    _inv(env)["WHEAT"] = 5

    place(env.state, farm, farm.farmer,
          PlaceActionState(type="PLACE", item="WHEAT", count=5), 0)

    assert env.state.private.shed.WHEAT == 100  # capped
    assert _inv(env).get("WHEAT") == 3  # excess stays in inventory


def test_place_shed_drop_noop_when_shed_full():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    env.state.private.shed.WHEAT = 100
    _inv(env)["CARROT"] = 5

    place(env.state, farm, farm.farmer,
          PlaceActionState(type="PLACE", item="CARROT", count=3), 0)

    assert env.state.private.shed.CARROT == 0  # nothing accepted
    assert _inv(env).get("CARROT") == 5  # all stays in inventory


def test_place_shed_drop_noop_when_not_shed_adjacent():
    env = _make_env(farmer=(0, 0))
    farm = env.state.farms[0]
    _inv(env)["WHEAT"] = 5
    ...
    assert env.state.private.shed.WHEAT == 0  # nothing dropped
    assert _inv(env).get("WHEAT") == 5


def test_place_shed_drop_noop_on_invalid_item():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    _inv(env)["BANANA"] = 5
...
def test_place_shed_drop_noop_when_inventory_empty():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
...
def test_place_shed_drop_counts_all_fields_toward_capacity():
    """Capacity is total items across all shed fields, not per-item."""
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    env.state.private.shed.WHEAT = 50
    env.state.private.shed.CARROT = 48  # 98 total; 2 slots left
    _inv(env)["MILK"] = 5

    place(env.state, farm, farm.farmer,
          PlaceActionState(type="PLACE", item="MILK", count=5), 0)

    assert env.state.private.shed.MILK == 2  # only 2 fit
...
def test_place_animal_priority_over_shed_drop():
    ...
    assert env.state.private.shed.GOOSE == 0  # not dropped into shed
    assert _inv(env).get("GOOSE") == 1


def test_place_non_matching_animal_falls_through_to_shed_drop():
    ...
    assert env.state.private.shed.GOOSE == 1  # dropped into shed
    assert _inv(env).get("GOOSE") == 1
...
def test_place_noop_on_malformed_or_negative_position(bad_pos):
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    _inv(env)["WHEAT"] = 5
...
def test_place_noop_out_of_bounds():
...
# ===========================================================================
# Integration: dispatch through Environment.step() reaches place.
# ===========================================================================

def test_step_dispatches_place_animal():
    ...
    assert env.state.private.inventories[0].get("GOOSE", 0) == 0


def test_step_dispatches_place_shed_drop():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    _inv(env)["WHEAT"] = 5
    ...
    assert env.state.private.shed.WHEAT == 3
    assert env.state.private.inventories[0].get("WHEAT") == 2