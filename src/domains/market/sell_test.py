...
class TestSell:
    ...
    def test_moves_items_from_shed_and_credits_money(self):
        env = _make_env()
    ...
    def test_adds_to_existing_market_inventory(self):
        env = _make_env()
        farm = env.state.farms[0]
    ...
    def test_accumulates_money_across_orders(self):
        env = _make_env()
        farm = env.state.farms[0]
        env.state.privates[0].shed.WHEAT = 5
        env.state.market.prices.WHEAT = 25
    ...
    def test_works_for_every_sellable_product(self, item):
        env = _make_env()
        farm = env.state.farms[0]
    ...
    def test_partial_when_count_exceeds_available(self):
        env = _make_env()
        farm = env.state.farms[0]
    ...
    def test_noop_when_shed_empty(self):
        env = _make_env()
        farm = env.state.farms[0]
    ...
    def test_noop_on_non_shed_item(self):
...
class TestSellDispatch:
    """Integration: sell dispatched through `Environment.step`."""

    def test_dispatches_sell_action(self):
        env = _make_env()
        farm = env.state.farms[0]
        env.state.privates[0].shed.WHEAT = 5
        env.state.market.prices.WHEAT = 25
    ...
    def test_sell_and_buy_seed_in_same_market_order(self):
