...
def test_buy_animal_one_buys_single_animal_at_fixed_cost(animal):
    env = _make_env()
...
def test_buy_animal_one_accumulates_across_calls():
    env = _make_env()
    farm, priv = _subjects(env)
...
def test_buy_animal_one_noop_when_cannot_afford():
    env = _make_env()
    farm, priv = _subjects(env)
    cost = ANIMAL_CONFIG["GOOSE"]["cost"]
    farm.money = 0.0
...
def test_buy_animal_one_partial_then_noop_when_money_runs_out():
    env = _make_env()
    farm, priv = _subjects(env)
    cost = ANIMAL_CONFIG["GOOSE"]["cost"]
