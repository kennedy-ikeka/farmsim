...
def test_market_in_state_is_market_controller():
    env = _make_env()
...
def test_apply_buy_seed_dispatches():
    env = _make_env()
    farm = env.state.farms[0]
    farm.money = 100.0
...
def test_apply_buy_product_dispatches():
    env = _make_env()
    farm = env.state.farms[0]
...
def test_apply_buy_animal_dispatches():
    env = _make_env()
    farm = env.state.farms[0]
    farm.money = 1000.0
...
def test_apply_hire_dispatches():
...
def test_apply_buy_land_dispatches():
