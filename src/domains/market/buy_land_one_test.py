...
def test_buy_land_one_unlocks_next_quadrant_in_order():
...
def test_buy_land_one_unlocks_in_sequence_across_calls():
    env = _env_with_locked_tiles()
    farm = _farm(env)
    farm.money = 10_000.0
...
def test_buy_land_one_noop_when_all_quadrants_unlocked():
    env = _env_with_locked_tiles()
    farm = _farm(env)
...
def test_buy_land_one_noop_when_cannot_afford():
    env = _env_with_locked_tiles()
    farm = _farm(env)
    farm.money = 0.0
...
def test_buy_land_one_partial_then_noop_when_money_runs_out():
    env = _env_with_locked_tiles()
    farm = _farm(env)
    # Afford NE (1000) only.
