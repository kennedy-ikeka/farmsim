...
def test_hire_one_charges_escalating_fib_cost(hires_so_far, expected_cost):
...
def test_hire_one_noop_when_cannot_afford():
    env = _make_env(farmer=(4, 4))
    farm = _farm(env)
    farm.money = 0.0
...
def test_hire_one_escalates_across_calls():
    env = _make_env(farmer=(4, 4))
    farm = _farm(env)
    farm.money = 100.0
...
def test_hire_one_spawns_at_shed_adjacent_tile():
    env = _make_env(farmer=(4, 4))
    farm = _farm(env)
    farm.money = 100.0
