...
def test_hire_deducts_escalating_cost(hires_so_far, expected_cost):
...
def test_hire_multiple_times_escalates_cost():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    farm.money = 100.0
...
def test_first_hire_spawns_at_sw_center_tile():
...
def test_second_hire_spawns_at_next_free_center_tile():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    farm.money = 100.0
...
def test_hire_picks_least_occupied_when_all_center_tiles_occupied():
...
def test_hire_noop_when_cannot_afford():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    farm.money = 0.0
...
def test_hire_noop_then_affordable_after_earning():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    farm.money = 0.0
...
# ---------------------------------------------------------------------------
# Integration: dispatch through Environment.step() reaches hire.
# ---------------------------------------------------------------------------

def test_step_dispatches_hire_action():
    env = _make_env(farmer=(4, 4))
    farm = env.state.farms[0]
    farm.money = 50.0
