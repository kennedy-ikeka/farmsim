...
def available_resources(player: RealityState) -> ResourceState:
    ...
    # STRUCTURE = count of built coops / pastures on the farm (occupied or not).
    structure_count = sum(
    ...
    return ResourceState(
        ...
        STRUCTURE=float(structure_count),
...
def generate_resource_needs(player: RealityState) -> ResourceState:
    ...
    Pipeline bottleneck conditions (active = nudge up, inactive = nudge down):
      ...
      STRUCTURE — animals in the shed but no empty structure to place them on.
    ...
    def adjust(field: str, active: bool) -> None:
        ...
        setattr(needs, field, min(1.0, cur + lr) if active else max(0.0, cur + lr))
    ...
    # STRUCTURE — animals in the shed but no empty structure to place them on.
    adjust("STRUCTURE", shed_animals > 0 and empty_structures == 0)
