...
# ---------------------------------------------------------------------------
# Config defaults + custom overrides.
# ---------------------------------------------------------------------------

def test_town_defaults_match_config():
    town = Town()
...
def test_town_accepts_custom_intervals():
...
# ---------------------------------------------------------------------------
# consume() orchestration — runs all three sub-ticks without advancing step.
# ---------------------------------------------------------------------------

def test_consume_does_not_advance_step():
...
def test_consume_runs_all_three_sub_ticks_on_combined_tick():
...
# ---------------------------------------------------------------------------
# Shop unlocks — day-based, random, monotonic, capped at all-shops.
# ---------------------------------------------------------------------------

def test_unlock_no_unlock_before_interval():
    env = _make_env(day=2)
    env.state.town.consume(env.state, _rng())
...
def test_unlock_fires_on_interval():
...
def test_unlock_grows_monotonically_and_does_not_repeat():
...
def test_unlock_no_unlock_when_all_shops_unlocked():
...
def test_unlock_respects_custom_interval():
    env = _make_env(day=5)
...
# ---------------------------------------------------------------------------
# Shop consumption — step-based, per-shop demand, 2x for single-product.
# ---------------------------------------------------------------------------

def test_shop_consume_no_tick_before_interval():
...
def test_shop_consume_fires_on_interval():
...
def test_shop_consume_multi_product_drops_one_of_each():
...
def test_shop_consume_single_product_shop_drops_two():
...
def test_shop_consume_pet_cafe_drops_two_carrots():
...
def test_shop_consume_all_single_product_shops_consume_2x(shop):
    env = _make_env(step=TOWN_SHOP_SELL_INTERVAL)
...
def test_shop_consume_multiple_shops_each_drop_their_demand():
...
def test_shop_consume_floors_inventory_at_zero():
    env = _make_env(step=TOWN_SHOP_SELL_INTERVAL)
    env.state.town.unlocked_shops = ["BAKERY"]
    _inv(env).WHEAT = 0
    _inv(env).EGG = 0
...
def test_shop_consume_no_shops_unlocked_is_noop():
    env = _make_env(step=TOWN_SHOP_SELL_INTERVAL)
...
def test_shop_consume_respects_custom_interval():
...
def test_shop_consume_repeated_ticks_accumulate():
...
# ---------------------------------------------------------------------------
# Town center consumption — step-based, day-scaled amount, excludes fertilizer.
# ---------------------------------------------------------------------------

def test_center_consume_no_tick_before_interval():
...
def test_center_consume_fires_on_interval():
...
def test_center_consume_excludes_fertilizer():
...
def test_center_consume_amount_one_through_day_10():
...
def test_center_consume_amount_two_after_day_10():
...
def test_center_consume_amount_two_through_day_20():
...
def test_center_consume_amount_four_after_day_20():
...
def test_center_amount_scales_with_day(day, expected):
    assert Town._center_amount(day) == expected


def test_center_consume_floors_inventory_at_zero():
    env = _make_env(step=TOWN_CENTER_SELL_INTERVAL, day=0)
    for p in TOWN_CENTER_PRODUCTS:
...
def test_center_consume_respects_custom_interval():
...
def test_center_consume_covers_all_products_except_fertilizer():
...
def test_step_runs_town_center_consume_on_interval():
    ...
    env.step(_turn(_step()))  # 13th call: town runs at step=12 → tick!
    for p in TOWN_CENTER_PRODUCTS:
        assert getattr(_inv(env), p) == 99
    assert _inv(env).FERTILIZER == 100  # untouched


def test_step_runs_shop_consume_when_shop_unlocked():
...
def test_step_unlocks_shop_on_day_rollover():
...
def test_step_town_runs_between_market_and_time_advance():
...
# ===========================================================================
# Event recording — town emits EventState records (player = -1).
# ===========================================================================
...
# ---------------------------------------------------------------------------
# No events when no tick fires or all ticks are no-ops.
# ---------------------------------------------------------------------------

def test_consume_no_events_when_no_tick_fires():
    env = _make_env(step=0, day=0)
    events = env.state.town.consume(env.state, _rng())
    assert events == []


def test_consume_no_events_when_inventory_all_zero():
...
def test_consume_no_shop_consume_event_when_no_shops_unlocked():
...
def test_shop_unlock_event_recorded_on_unlock():
    env = _make_env(day=TOWN_SHOP_UNLOCK_INTERVAL)
    events = env.state.town.consume(env.state, _rng())
...
def test_shop_unlock_event_step_day_hour_match_state():
...
def test_shop_unlock_no_event_when_saturated():
...
def test_shop_consume_event_recorded_on_tick():
    env = _make_env(step=TOWN_SHOP_SELL_INTERVAL)
    env.state.town.unlocked_shops = ["BAKERY"]
    _inv(env).EGG = 50
    _inv(env).WHEAT = 50
...
def test_shop_consume_event_aggregates_across_shops():
...
def test_shop_consume_event_single_product_records_two():
...
def test_shop_consume_event_floors_at_actual_inventory():
...
# ---------------------------------------------------------------------------
# CENTER_CONSUME events.
# ---------------------------------------------------------------------------

def test_center_consume_event_recorded_on_tick():
    env = _make_env(step=TOWN_CENTER_SELL_INTERVAL, day=0)
...
def test_center_consume_event_records_amount_per_product():
    env = _make_env(step=TOWN_CENTER_SELL_INTERVAL, day=21)
    env.state.town.town_shop_unlock_interval = 10_000
    env.state.town.town_shop_sell_interval = 10_000
    for p in TOWN_CENTER_PRODUCTS:
...
def test_center_consume_event_excludes_fertilizer():
    env = _make_env(step=TOWN_CENTER_SELL_INTERVAL, day=0)
    env.state.town.town_shop_unlock_interval = 10_000
    env.state.town.town_shop_sell_interval = 10_000
    _inv(env).FERTILIZER = 100
...
def test_center_consume_event_floors_at_actual_inventory():
...
# ---------------------------------------------------------------------------
# Combined event ordering — unlock → shop consume → center consume.
# ---------------------------------------------------------------------------

def test_consume_returns_events_in_unlock_then_shop_then_center_order():
...
def test_consume_returns_empty_list_when_all_ticks_noop():
...
# ---------------------------------------------------------------------------
# Integration — Environment.step records town events in self.events.
# ---------------------------------------------------------------------------

def test_step_records_center_consume_event():
...
def test_step_records_shop_unlock_event_on_day_rollover():
    env = _make_env(step=0, day=0)
    for _ in range(73):
        env.step(_turn(_step()))
...
def test_step_records_shop_consume_event_when_shop_unlocked():
    env = _make_env(step=0, day=0)
    env.state.town.unlocked_shops = ["BAKERY"]
    _inv(env).EGG = 100
    _inv(env).WHEAT = 100
...
def test_step_town_events_carry_player_minus_one():
...
def test_step_town_events_do_not_break_player_tagged_events():
